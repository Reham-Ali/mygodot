...
def make_certs_header(target, source, env):
    buffer = methods.get_buffer(source[0])
...
constexpr inline int _certs_compressed_size = {len(buffer)};
constexpr inline int _certs_uncompressed_size = {decomp_size};
constexpr inline unsigned char _certs_compressed[] = {{
    ...
    with methods.generated_wrapper(target[0]) as file:
        ...
        for line in buffer.decode().splitlines():
            if line.startswith("    ") and reading:
                ...
                if section:
                    file.write(f"constexpr inline const char *{section}[] = {{\n")
                    reading = True
...
def make_license_header(target, source, env):
    ...
    with methods.generated_wrapper(target[0]) as file:
        file.write(f"""\
constexpr inline const char *GODOT_LICENSE_TEXT = {{
...
""")
        ...
        file.write("constexpr inline const char *COPYRIGHT_INFO_DATA[] = {\n")
        for line in data_list:
            file.write(f'\t"{methods.to_escaped_cstring(line)}",\n')
        file.write("};\n\n")

        file.write("constexpr inline ComponentCopyrightPart COPYRIGHT_PROJECT_PARTS[] = {\n")
        part_index = 0
        part_indexes = {}
        ...
        file.write(f"constexpr inline int COPYRIGHT_INFO_COUNT = {len(projects)};\n")

        file.write("constexpr inline ComponentCopyright COPYRIGHT_INFO[] = {\n")
        for project_name, project in iter(projects.items()):
            file.write(
        ...
        file.write(f"constexpr inline int LICENSE_COUNT = {len(license_list)};\n")

        file.write("constexpr inline const char *LICENSE_NAMES[] = {\n")
        for license in license_list:
            file.write(f'\t"{methods.to_escaped_cstring(license[0])}",\n')
        file.write("};\n\n")

        file.write("constexpr inline const char *LICENSE_BODIES[] = {\n\n")
        for license in license_list:
            to_raw = []
