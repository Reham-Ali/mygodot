...
def make_doc_header(target, source, env):
    buffer = b"".join([methods.get_buffer(src) for src in source])
    decomp_size = len(buffer)
    buffer = methods.compress_buffer(buffer)

    with methods.generated_wrapper(target[0]) as file:
        file.write(f"""\
constexpr inline const char *_doc_data_hash = "{hash(buffer)}";
constexpr inline int _doc_data_compressed_size = {len(buffer)};
constexpr inline int _doc_data_uncompressed_size = {decomp_size};
constexpr inline const unsigned char _doc_data_compressed[] = {{
...
def make_translations_header(target, source, env):
    ...
    with methods.generated_wrapper(target[0], prefix=category) as file:
        for path in sorted_paths:
            ...
            file.write(f"""\
constexpr inline const unsigned char _{category}_translation_{name}_compressed[] = {{
	{methods.format_buffer(buffer, 1)}
}};
...
constexpr inline {category.capitalize()}TranslationList _{category}_translations[] = {{
""")
